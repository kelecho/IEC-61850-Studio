...
/// Construye un AARQ que envuelve el `Initiate-RequestPDU` de MMS.
...
pub fn aare(initiate_response: &[u8]) -> Vec<u8> {
    ...
    w.tlv(AARE, |w| {
        ...
        // result [2] IMPLICIT INTEGER 0 (accepted)
...
pub fn parse_aare(data: &[u8]) -> Result<&[u8], MmsError> {
    ...
    // result [2] IMPLICIT Associate-result (INTEGER: accepted(0), rejected-permanent(1),
    // rejected-transient(2)).
    if let Some(result_tlv) = find_child(outer.content, Tag::context(2, false))? {
        let result = crate::ber::prim::decode_integer(result_tlv.content)?;
        if result != 0 {
